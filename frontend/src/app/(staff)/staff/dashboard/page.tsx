    ...
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {statsLoading ? (
          ...
          <>
            <StatCard label="Assigned to Me" value={stats?.open ?? 0} icon={<Ticket className="w-5 h-5" />} color="bg-primary-50 text-primary-700" />
            <StatCard label="In Progress" value={stats?.in_progress ?? 0} icon={<Clock className="w-5 h-5" />} color="bg-amber-100 text-amber-700" />
            <StatCard label="Resolved Today" value={stats?.resolved ?? 0} icon={<CheckCircle2 className="w-5 h-5" />} color="bg-success-bg text-success" />
            <StatCard label="SLA Breached" value={stats?.sla_breached ?? 0} icon={<AlertTriangle className="w-5 h-5" />} color="bg-danger-bg text-danger" />
          </>
        )}
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-display font-semibold text-gray-900 text-lg">My Ticket Queue</h2>
          <Link href="/staff/tickets" className="text-sm text-primary-700 hover:text-primary-900 font-medium">
            View all →
          </Link>
        </div>
